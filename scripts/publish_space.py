...
two places: the README starts with the header Spaces require, and the
Dockerfile sets RAG_PLAYGROUND_DEMO=1. Log in first with `hf auth login`.
"""
...
def space_dockerfile(dockerfile: str) -> str:
    head, sep, tail = dockerfile.rpartition("CMD ")
    return f"{head}ENV RAG_PLAYGROUND_DEMO=1\n{sep}{tail}"
...
def export_tree(dest: Path) -> list[str]:
    ...
    for name, change in (("README.md", space_readme), ("Dockerfile", space_dockerfile)):
        path = dest / name
